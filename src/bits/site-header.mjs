// [fo] — sitewide top header: left links | brand | right links
// left/right are arrays of { title, href }
export function siteHeader({ left, right }) {
  const links = items => items.map(l => `<a href="${l.href}" class="sh-link">${l.title}</a>`).join('');
  return `<header class="site-header">
  <div class="site-header-inner">
    <div class="sh-left">${links(left)}</div>
    <a href="index.html" class="sh-brand">
      <svg class="sh-icon" width="28" height="32" viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <path fill="currentColor" d="M448 336v-288C448 21.49 426.5 0 400 0H96C42.98 0 0 42.98 0 96v320c0 53.02 42.98 96 96 96h320c17.67 0 32-14.33 32-31.1c0-11.72-6.607-21.52-16-27.1v-81.36C441.8 362.8 448 350.2 448 336zM144 144c0-8.875 7.125-15.1 16-15.1L208 128V80c0-8.875 7.125-15.1 16-15.1l32 .0009c8.875 0 16 7.12 16 15.1V128L320 128c8.875 0 16 7.121 16 15.1v32c0 8.875-7.125 16-16 16L272 192v112c0 8.875-7.125 16-16 16l-32-.0002c-8.875 0-16-7.127-16-16V192L160 192c-8.875 0-16-7.127-16-16V144zM384 448H96c-17.67 0-32-14.33-32-32c0-17.67 14.33-32 32-32h288V448z"/>
      </svg>
      <span class="sh-title">הגיון לבי</span>
    </a>
    <div class="sh-right">${links(right)}</div>
  </div>
</header>`;
}
