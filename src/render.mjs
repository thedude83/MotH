...
export function render(src, css, records = {}, headerData = null) {
  ...
  const pageLinks = nodes
    .filter(n => n.name === 'nav-link')
    .map(n => ({ href: '/' + n.rest + '.html', label: slotOf(n, 'title') }));

  const body = nodes
    .filter(n => n.name !== 'page' && n.name !== 'nav-link')
    .map(renderNode)
    .join('\n');

  return shell({ pd, sections, css, body, headerData, pageLinks });
}

function shell({ pd, sections, css, body, headerData, pageLinks = [] }) {
...
<style>${css}</style>
<script>
document.documentElement.dataset.theme=localStorage.getItem('moth-theme')||'${headerData?.theme||'dark'}';
document.documentElement.style.zoom=localStorage.getItem('moth-zoom')||'1';
</script>
</head>
<body>
${headerData ? siteHeader(headerData) : ''}
${isContent ? nav({ brand, sections, pageLinks }) : ''}
<div class="container">
${hero({ ...pd, sections })}
${enterLink}
...
(function(){
  ...
  applyTheme(localStorage.getItem('moth-theme')||'${headerData?.theme||'dark'}');
